...
struct ContentView: View {
    ...
    @AppStorage("wiki.qaq.ifixitx.restartLaunchAgents")
    var restartLaunchAgents: Bool = true
    @AppStorage("wiki.qaq.ifixitx.restartLoginItems")
    var restartLoginItems: Bool = true
    ...
    var options: some View {
        ForEach([toggleIds], id: \.self) { _ in
            VStack(alignment: .leading, spacing: 8) {
                ...
                Toggle("Restart LaunchAgents", isOn: $restartLaunchAgents)
                Toggle("Restart Login Items", isOn: $restartLoginItems)
    ...
    func _executeRestart() {
        ...
        // 重新加载 LaunchAgents
        if restartLaunchAgents && !launchAgentPlists.isEmpty {
        ...
        // 启动 Login Items
        if restartLoginItems && !loginItemsList.isEmpty {
